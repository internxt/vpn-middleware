...
const verifyToken = (token: string) => {
  ...
  const tokenObject = JSON.parse(
    Buffer.from(tokenBuffer, 'base64').toString('utf-8'),
  );
...
export const bootstrapServer = async () => {
  const server = http.createServer(async (req, res) => {
    ...
    const connectionHandler = new ProxyRequestHandler({
      proxyUrl: proxyList[0].url,
      proxyAuth: proxyList[0].auth,
      req,
      res,
    });
  ...
  server.on('connect', (req, socket, head) => {
    const connectionHandler = new ProxyConnectHandler({
      proxyUrl: proxyList[0].url,
      proxyAuth: proxyList[0].auth,
      clientSocket: socket,
      req,
